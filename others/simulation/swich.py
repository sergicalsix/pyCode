...
def multi_simulation(k1,k2,k3,k4,s0,g0,delta_time,iteration,comment = 'multi_sim1')->List[List[float]]:
    ...
    sim_param_list = copy.deepcopy(param_list) #実際にシミュレーションで参照するパラメータリスト: sim_param_list 
    ...
    for i,param in enumerate(params):
        ...
        g_multi_list[i]  = simulation(*sim_param_list,delta_time,iteration,comment= comment + '_' + str(param))
        ic(g_multi_list[i][-1])
...
def plot_multi_simulation(time_list,value_multi_list,param_list,param_label = 'g0' ,fig_name = 'multi_sim1')->None:
    for param,value_list in zip(param_list,value_multi_list):
        plt.scatter(time_list,value_list,label = param_label + '=' +  str(param))
    plt.xlabel('time')
    plt.ylabel('g')
    if param_label!= "all":
        plt.legend()
...
def main():
    ...
    s0,g0 = 1,1
    # シミュレーションの条件の設定
    delta_time,total_time = 0.1, 10
    iteration = round(total_time / delta_time)
    time_list = [i * delta_time for i in range(iteration)]
    # 単一シミュレーション
    g_list = simulation(k1,k2,k3,k4,s0,g0,delta_time,iteration,comment='sim1')
    plot_simulation(time_list,g_list,fig_name = 'sim1')

    ## gの初期値を変更してシミュレーション
    g0_list = [1,5,10,20]
    g_multi_list = multi_simulation(k1,k2,k3,k4,s0,g0_list,delta_time,iteration,comment = 'multi_sim_g0')
    plot_multi_simulation(time_list,g_multi_list,g0_list,param_label = 'g0' ,fig_name = 'multi_sim_g0')

    ## k1を変更してシミュレーション
    k1_list = [1,5,10,20]
    g_multi_list = multi_simulation(k1_list,k2,k3,k4,s0,g0,delta_time,iteration,comment = 'multi_sim_k1')
    plot_multi_simulation(time_list,g_multi_list,k1_list,param_label = 'k1' ,fig_name = 'multi_sim_k1')

    ## k2を変更してシミュレーション
    k2_list = [1,5,10,20]
    g_multi_list = multi_simulation(k1,k2_list,k3,k4,s0,g0,delta_time,iteration,comment = 'multi_sim_k2')
    plot_multi_simulation(time_list,g_multi_list,k2_list,param_label = 'k2' ,fig_name = 'multi_sim_k2')

    ## k3を変更してシミュレーション
    k3_list = [1,5,10,20]
    g_multi_list = multi_simulation(k1,k2,k3_list,k4,s0,g0,delta_time,iteration,comment = 'multi_sim_k3')
    plot_multi_simulation(time_list,g_multi_list,k3_list,param_label = 'k3' ,fig_name = 'multi_sim_k3')

    ## k4を変更してシミュレーション
    k4_list = [1,5,10,20]
    g_multi_list = multi_simulation(k1,k2,k3,k4_list,s0,g0,delta_time,iteration,comment = 'multi_sim_k4')
    plot_multi_simulation(time_list,g_multi_list,k4_list,param_label = 'k4' ,fig_name = 'multi_sim_k4')
